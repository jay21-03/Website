...
function Layout({ children }) {
  ...
  return <><header className="site-header"><Link className="brand" to="/"><span className="pot-mark">◯</span>Đàng Xem</Link><nav className="main-nav"><NavLink to="/">{pick(lang, 'Trang chủ', 'Home')}</NavLink><NavLink to="/products">{pick(lang, 'Sản phẩm', 'Products')}</NavLink><NavLink to="/collections">{pick(lang, 'Bộ sưu tập', 'Collections')}</NavLink><NavLink to="/workshop">{pick(lang, 'Workshop', 'Workshop')}</NavLink><NavLink to="/support">{pick(lang, 'Hỗ trợ', 'Support')}</NavLink>{user?.role === 'USER' && <NavLink to="/orders">{pick(lang, 'Đơn hàng', 'Orders')}</NavLink>}{user?.role === 'ADMIN' && <NavLink to="/admin">{pick(lang, 'Quản trị', 'Admin')}</NavLink>}</nav><div className="header-tools"><button className="lang-btn" type="button" onClick={() => setLang(lang === 'vi' ? 'en' : 'vi')}>{lang === 'vi' ? 'EN' : 'VI'}</button>{user?.role !== 'ADMIN' && <Link className="icon-btn cart-btn" to="/cart">{pick(lang, 'Giỏ', 'Cart')} <span className="cart-count">{cart.items.reduce((sum, item) => sum + item.quantity, 0)}</span></Link>}{user ? <Link className="icon-btn" to={user.role === 'ADMIN' ? '/admin' : '/account'}>{user.role === 'ADMIN' ? pick(lang, 'Quản trị', 'Admin') : user.fullName}</Link> : <DirectGoogleLogin />}</div></header>{children}<BilingualFooter /><div className="floating-chat">{support.phones[0] && <a className="zalo" href={`https://zalo.me/${support.phones[0]}`} target="_blank" rel="noreferrer" aria-label={`Zalo ${support.phones[0]}`} title={`Zalo ${support.phones[0]}`}>Z</a>}{support.facebook && <a className="facebook" href={support.facebook} target="_blank" rel="noreferrer" aria-label="Facebook" title="Facebook">f</a>}</div></>
}
...
function ProductCard({ product, onView }) {
  ...
  return <article className="product-card"><Link className="product-image" to={`/products/${product.id}`}><img src={product.thumbnailUrl || fallbackImage} alt={name} onError={event => { event.currentTarget.src = fallbackImage }} /></Link><div className="product-info"><div className="product-top"><div><h3>{name}</h3><p>{lang === 'vi' ? product.nameEn : product.nameVi}</p></div><span className="price">{money(product.sellingPrice, lang)}</span></div>{product.sellingPrice < product.basePrice && <p><s>{money(product.basePrice, lang)}</s></p>}<div className="card-actions"><button onClick={() => onView(product)}>{pick(lang, 'Xem nhanh', 'Quick view')}</button><button className="add-cart" onClick={() => add(product.id)}>{pick(lang, 'Thêm vào giỏ', 'Add to cart')}</button></div></div></article>
}
...
function ProductDetail() {
  ...
  if (productQuery.isLoading) return <Layout><main className="page"><Loading /></main></Layout>
  if (productQuery.isError) return <Layout><main className="page"><div className="state-box error" role="alert">{productQuery.error.message}<button onClick={() => productQuery.refetch()}>Thử lại</button></div></main></Layout>
  if (!product) return <Layout><main className="page"><Empty>Không tìm thấy sản phẩm.</Empty></main></Layout>
  const name = lang === 'vi' ? product.nameVi : product.nameEn
  const description = lang === 'vi' ? product.descriptionVi : product.descriptionEn
...
function Cart() {
  ...
  return <Layout><main className="page"><div className="page-title"><span className="kicker">Giỏ hàng</span><h1>Giỏ hàng của bạn</h1></div>{authLoading ? <Loading /> : !user || user.role !== 'USER' ? <Empty>Bạn cần đăng nhập bằng tài khoản khách hàng để dùng giỏ hàng.</Empty> : !cart.items.length ? <Empty>Giỏ hàng đang trống. <Link to="/products">Tiếp tục mua sắm</Link></Empty> : <div className="checkout-grid"><section className="order-summary cart-panel">{cart.items.map(item => <div className="checkout-item" key={item.id}><img src={item.thumbnailUrl || fallbackImage} alt="" /><div><b>{lang === 'vi' ? item.nameVi : item.nameEn}</b><span>Còn có thể đặt: {item.availableQuantity}</span><span><button type="button" onClick={() => change(item, item.quantity - 1)}>−</button> {item.quantity} <button type="button" disabled={item.quantity >= item.availableQuantity} onClick={() => change(item, item.quantity + 1)}>+</button></span><button type="button" className="link-danger" onClick={() => change(item, 0)}>Xóa</button></div><div><small>{money(item.sellingPrice, lang)} × {item.quantity}</small><b>{money(item.lineTotal, lang)}</b></div></div>)}<div className="summary-line"><span>Tạm tính</span><b>{money(cart.totalAmount, lang)}</b></div><div className="summary-line total"><span>Tổng cộng</span><b>{money(cart.totalAmount, lang)}</b></div><Link className="button dark full" to="/checkout">Tiếp tục thanh toán</Link></section></div>}</main></Layout>
}
function Checkout() {
...
function OrderDetail() {
  ...
  return <Layout><main className="page"><div className="page-title"><span className="kicker">Chi tiết đơn hàng</span><h1>{order?.orderCode || 'Đơn hàng'}</h1></div>{authLoading || query.isLoading ? <Loading /> : !user || user.role !== 'USER' ? <Empty>Vui lòng đăng nhập bằng tài khoản khách hàng.</Empty> : query.isError && query.error.status === 404 ? <Empty>Không tìm thấy đơn hàng.</Empty> : query.isError ? <div className="state-box error" role="alert">{query.error.message}<button onClick={() => query.refetch()}>Thử lại</button></div> : <div className="order-detail"><section><h3>Thông tin nhận hàng</h3><p>{order.receiverName} · {order.phone}</p>{order.email && <p>{order.email}</p>}<p>{order.address}</p>{order.note && <p>Ghi chú: {order.note}</p>}<p>Ngày tạo: {date(order.createdAt, lang)}</p><p>Đơn hàng: <b>{orderStatusLabel(order.orderStatus, lang)}</b></p><p>Thanh toán: <b>{paymentStatusLabel(order.paymentStatus, lang)}</b></p>{pendingPayment && <small>Trang này tự cập nhật trạng thái thanh toán mỗi 5 giây.</small>}<div className="payment-panel"><h3>Thanh toán payOS</h3><div className="summary-line"><span>Trạng thái</span><b>{paymentStatusLabel(order.paymentStatus, lang)}</b></div><div className="summary-line"><span>Số tiền thanh toán</span><b>{money(order.paymentAmount ?? order.totalAmount, lang)}</b></div>{order.expiresAt && <div className="summary-line"><span>Hết hạn</span><b>{date(order.expiresAt, lang)}</b></div>}{order.qrCode && <img className="payment-qr" src={order.qrCode} alt="Mã QR thanh toán payOS" />}{order.checkoutUrl ? <a className="button dark full" href={order.checkoutUrl} target="_blank" rel="noreferrer">Thanh toán qua payOS</a> : pendingPayment && <p className="payment-note">Cổng thanh toán đang chờ cấu hình hoặc thông tin thanh toán chưa khả dụng. Trạng thái đơn hàng vẫn được lấy từ hệ thống.</p>}</div></section><aside className="order-summary">{order.items.map(item => <div className="summary-line" key={item.productId}><span>{lang === 'vi' ? item.productNameVi : item.productNameEn} × {item.quantity}<small>{money(item.sellingPrice, lang)} / sản phẩm</small></span><b>{money(item.totalPrice, lang)}</b></div>)}<div className="summary-line"><span>Tạm tính</span><b>{money(order.subtotal, lang)}</b></div><div className="summary-line total"><span>Tổng cộng</span><b>{money(order.totalAmount, lang)}</b></div></aside></div>}</main></Layout>
}
function Faq() { const { lang } = useStore(); return <Layout><main className="page support-page"><div className="page-title"><span className="kicker">FAQ</span><h1>{pick(lang, 'Câu hỏi thường gặp', 'Frequently asked questions')}</h1></div><section className="faq-list"><details open><summary>Gốm Bàu Trúc có dùng bàn xoay không?</summary><p>Không. Sản phẩm được tạo hình bằng tay theo kỹ thuật truyền thống của người Chăm.</p></details><details><summary>Tôi có thể đặt lịch workshop không?</summary><p>Có. Mở trang Workshop, chọn gói phù hợp và gửi yêu cầu đặt lịch để cửa hàng xác nhận.</p></details><details><summary>Thanh toán online hoạt động thế nào?</summary><p>Hệ thống tạo Payment trạng thái PENDING và hiển thị thông tin payOS nếu cổng thanh toán đã được cấu hình.</p></details></section></main></Layout> }
function Policy() { return <Layout><main className="page support-page"><div className="page-title"><span className="kicker">Chính sách</span><h1>Chính sách mua hàng</h1></div><section className="policy"><h2>Thanh toán</h2><p>Đơn hàng sử dụng thanh toán payOS. Ứng dụng không tự đánh dấu thanh toán thành công; trạng thái được cập nhật từ hệ thống backend.</p><h2>Hoàn tiền thủ công</h2><p>Hoàn tiền tự động ngoài phạm vi. Khi cần, admin chỉ ghi nhận khoản hoàn tiền đã thực hiện bên ngoài hệ thống.</p><h2>Vận chuyển</h2><p>Phí vận chuyển và theo dõi vận chuyển chưa thuộc phạm vi hệ thống hiện tại.</p></section></main></Layout> }
function Workshop() {
...
function AdminPortal() {
  ...
  return <Admin user={store.user} notify={store.notify} onLogout={logout} />
}

export default function App() { return <StoreProvider><Routes><Route path="/" element={<Home />} /><Route path="/products" element={<Products />} /><Route path="/products/:id" element={<ProductDetail />} /><Route path="/collections" element={<CollectionsPage />} /><Route path="/workshop" element={<Workshop />} /><Route path="/login" element={<Account />} /><Route path="/account" element={<Account />} /><Route path="/cart" element={<Cart />} /><Route path="/checkout" element={<Checkout />} /><Route path="/orders" element={<Orders />} /><Route path="/orders/:id" element={<OrderDetail />} /><Route path="/support" element={<Support />} /><Route path="/faq" element={<Faq />} /><Route path="/policy" element={<Policy />} /><Route path="/admin" element={<AdminPortal />} /><Route path="*" element={<Home />} /></Routes></StoreProvider> }
